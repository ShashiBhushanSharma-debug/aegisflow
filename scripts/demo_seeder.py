...
SCENARIOS = [
    ("WIN", "chargeback_fraud", 250000),
    ("LOSE", "product_not_received", 189900),
    ("WIN", "product_not_received", 429000),
    ("LOSE", "chargeback_fraud", 315000),
    ("WIN", "product_unacceptable", 129500),
    ("LOSE", "credit_not_processed", 275000),
]
...
def main() -> None:
    ...
    if args.once:
        pick = next(s for s in SCENARIOS if s[0] == args.once)
        send(secret, *pick)
        return

    for i in range(args.count):
        send(secret, *SCENARIOS[i % len(SCENARIOS)])
        if i < args.count - 1:
